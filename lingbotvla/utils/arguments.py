...
class ModelArguments:
    ...
    config_key: Literal["LingbotVLAConfig", "GrootConfig", "LingbotVLAV2Config"] = field(
        default="LingbotVLAConfig",
        metadata={"help": "Which config to use."},
    )
    vit_attn_implementation: str = field(
        default="flash_attention_2",
        metadata={"help": "_attn_implementation in vit config"},
    )
...
class DataArguments:
    ...
    state_norm_type: str = field(
        default="none",
        metadata={
            "help": (
                "Normalization type for VLA state features. Use 'none' to reuse "
                "`norm_type`; use 'sincos' to encode raw state angles as cos/sin "
                "while actions still use `norm_type`."
            )
        },
    )
    image_augment: bool = field(
        default=False,
        metadata={
            "help": (
                "Enable training-time image augmentation for VLA datasets. "
                "Random color parameters are sampled once per sample "
                "and replayed across all camera views, matching the RLDX/Cosmos style."
            )
        },
    )
    def __post_init__(self):
...
class TrainingArguments:
    ...
    muon_nesterov: bool = field(
        default=True,
        metadata={"help": "Whether Muon applies Nesterov-style lookahead momentum."},
    )
    muon_ns_steps: int = field(
        default=5,
        metadata={"help": "Newton-Schulz iteration count for Muon's orthogonalization."},
    )
    muon_adjust_lr_fn: Optional[str] = field(
        default="match_rms_adamw",
        metadata={"help": "Muon LR adjustment: None | 'original' | 'match_rms_adamw'."},
    )
    muon_exclude_name_patterns: Optional[List[str]] = field(
        default_factory=list,
        metadata={
            "help": (
                "Substrings of parameter FQNs that should be forced to AdamW even if "
    ...
    allow_partial_checkpoint: bool = field(
        default=False,
        metadata={"help": "Allow partial checkpoint loading (skip missing keys). Useful when resuming from a checkpoint with different model structure."},
    )
    ...
    async_save_hf_weights: bool = field(
        default=False,
        metadata={"help": "Save huggingface format weights on rank 0 in a best-effort background thread."},
    )
    async_hf_max_pending: int = field(
        default=1,
...
class EvalArguments:
    enabled: Optional[bool] = field(
        default=None,
        metadata={"help": "Whether to submit an eval job after HF checkpoint conversion."},
    )
    command: Optional[str] = field(
        default=None,
        metadata={"help": "Raw command executed by the eval job. Required when eval submission is enabled."},
    )
    env: Dict[str, Any] = field(
        default_factory=dict,
        metadata={"help": "Extra environment variables exported before running eval.command."},
    )
    eval_step: Optional[int] = field(
        default=None,
        metadata={"help": "Submit eval only after global_step >= eval_step. None or <= 0 means no step gate."},
    )
    eval_epoch: Optional[int] = field(
        default=None,
        metadata={"help": "Submit eval only after epoch >= eval_epoch. None or <= 0 means no epoch gate."},
    )
    image: Optional[str] = field(
        default=None,
        metadata={"help": "Docker image used by the eval job."},
    )
    k8s_app_name: Optional[str] = field(
        default=None,
        metadata={"help": "K8S app name used by the eval job."},
    )
    k8s_priority: Optional[str] = field(
        default=None,
        metadata={"help": "K8S priority used by the eval job."},
    )
    worker_num: Optional[int] = field(
        default=None,
        metadata={"help": "Number of eval workers."},
    )
    cpu: Optional[int] = field(
        default=None,
        metadata={"help": "CPU cores requested by the eval worker."},
    )
    memory: Optional[int] = field(
        default=None,
        metadata={"help": "Memory in MB requested by the eval worker."},
    )
    gpu_num: Optional[int] = field(
        default=None,
        metadata={"help": "Number of GPUs requested by the eval worker."},
    )
    gpu_percent: Optional[int] = field(
        default=None,
        metadata={"help": "GPU percent requested by the eval worker."},
    )
    gpu_type: Optional[str] = field(
        default=None,
        metadata={"help": "GPU type requested by the eval worker."},
    )
    disk_m: Optional[int] = field(
        default=None,
        metadata={"help": "Disk in MB requested by the eval worker."},
    )
    shared_memory: Optional[int] = field(
        default=None,
        metadata={"help": "Shared memory in MB requested by the eval worker."},
    )
    datastore_mount: Optional[List[str]] = field(
        default=None,
        metadata={"help": "Datastore mount points for the eval job."},
    )
    datastore_name: Optional[List[str]] = field(
        default=None,
        metadata={"help": "Datastore names for the eval job."},
    )
    datastore_sub_path: Optional[List[str]] = field(
        default=None,
        metadata={"help": "Datastore sub paths for the eval job."},
    )
    repo_url: Optional[str] = field(
        default=None,
        metadata={"help": "Code repository URL used by the eval job."},
    )
    repo_branch: Optional[str] = field(
        default=None,
        metadata={"help": "Code repository branch used by the eval job."},
    )
    repo_commit_id: Optional[str] = field(
        default=None,
        metadata={"help": "Code repository commit ID used by the eval job."},
    )
    task_name: Optional[str] = field(
        default=None,
        metadata={"help": "Eval task name. If empty, a unique name is generated."},
    )
    workdir: Optional[str] = field(
        default=None,
        metadata={"help": "Optional working directory inside the eval job."},
    )
...
def save_args(args: T, output_path: str) -> None:
    ...
    args_dict.pop("eval", None)
    with open(local_path, "w") as f:
        f.write(yaml.safe_dump(args_dict, default_flow_style=False))
